    ...
    <table border="1">
        <tr>
            <td>
                <c:if test="${pageable.sortBy== 'desc'}">
                    <a href="/admin/users?page=${pageable.page}&search=${pageable.search}&sortby=asc&nameField=customers.id">
                        Id
                    </a>
                </c:if>
                <c:if test="${pageable.sortBy== 'asc'}">
                    <a href="/admin/users?page=${pageable.page}&search=${pageable.search}&sortby=desc&nameField=customers.id">
                        Id
                    </a>
                </c:if>
            </td>
        ...
        <c:forEach items="${users}" var="customer">
            <tr>
                ...
                <td><a href="/admin/users?action=edit&id=${customer.id}">Edit</a> </td>
                <td><a href="/admin/users?action=delete&id=${customer.id}" onclick="return confirm('Do you want to remove ${customer.name}?')">Delete</a> </td>
            </tr>
        </c:forEach>
    </table>
    <ul>
